...
The data on which the analyzes are made is composed of four data sets that were concatenated together. They all contain the match results for four English premier league seasons (2018-2019, 2019-2020, 2020-2021, 2021-2022).
...
The variables that we mostly use are the ones regarding match results. We build two models regarding two different definitions of the home advantage. First, we consider it as the fact that a team will goal more or less if they play at home. Secondly, we consider it as the fact that a team is more or less likely to win if they play at home. 
...
We can fit the previous model only for the 2019-2020 season which is the most impacted by covid-19. The diagnostics we obtain are presented in *Figure 2*. The residual plot shows two distinct bands which correspond to the two values of the variable *team status* : Home team corresponding to playing home, and Away team corresponding to playing away. The normal QQ-plot shows that the residuals are skewed with some outliers. One could refit the model without those influential outliers to see to what extent the model is changed. The scale location and constant leverage also enable to identify those outliers.
...
By fitting the model for all the seasons together (which captures an overall positive effect of playing home even though the effect is not present during covid), we obtain the diagnostics displayed in *Figure 3*. They globally have the same shape as for the model fitted only on the 2019-20 season. Thus, we again identify some influential outliers, mostly on the top right of the QQplot.
