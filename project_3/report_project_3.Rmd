...
The figure indicates that there is a slight advantage to playing at home since the average difference in the number of goals scored is positive. It appears that the COVID-19 pandemic had little impact on the average difference in the number of goals scored, although there are fewer extreme scores during the closed-doors matches. In the next section, we model the data to gain a deeper understanding of the extent to which the home advantage persisted during the COVID-19 pandemic. As a first definition of home advantage, we primarily examine the impact of playing at home on a team's goal scoring ability. Secondly, we use the match results (win or loose) to build a poisson model. Our analysis aims to provide robust evidence on the impact of playing without fans and whether it affected the home advantage in the English Premier League.
...
$N_{goals} = \gamma_{0} +  \gamma_{1}Home + \epsilon$
...
The equation for this model can be written 
$log(\mathbb{E}(N_{goals})) = \beta_{0} + \beta_{1}Home + \zeta$

where $\beta_{1}$ is the estimated coefficient associated to playing home rather than away, $\zeta$ is the residual, $Home$ denotes the fact that the teams plays home and $N_{goals}$ is the number of goals.


# 4. Residuals diagnostics
We fit the previously mentioned Poisson model on all the seasons together (which captures an overall positive effect of playing at home, even though the effect is not present during the COVID-19 pandemic). The model diagnostics are presented in *Figure 2*. The residual plot displays two distinct bands, which correspond to the two values of the variable "team status" : home team (playing at home) and away team (playing away). The normal QQ-plot indicates that the residuals are skewed with some outliers. It may be worthwhile to refit the model without these influential outliers to determine the extent to which the model is impacted. The scale-location and constant leverage plots also identify these outliers.


```{r, fig.align="center", fig.cap="Figure 2 : Residuals diagnostics for the model considering all seasons"}
par(mfrow=c(2,2))
...
# 6. Interpretation
In section 3, we present two models to quantify the home advantage. The first model examines the difference in the number of goals between playing at home and playing away. Before the COVID-19 pandemic, we observe a significant positive coefficient associated with playing at home, indicating a clear home advantage. The coefficient is practically significant, with a value of 0.27 (95CI = [0.14, 0.4]), which means that teams score 0.27 more goals when playing at home. However, during the pandemic, the coefficient associated with playing at home is not statistically significant since 0 belongs to the confidence interval, indicating no significant effect of playing at home. After the closed-door period, the effect of playing at home becomes statistically and practically significant again.
...
By taking in account the promoted and relegated teams, as well as the draw effect, we obtain estimated coefficients of the same magnitude and statistical significance. The model is therefore stable which gives additional confidence in its accuracy. 


# 7. Discussion
The models developed show a decrease in home advantage during the COVID-19 pandemic due to the closure of public stadiums. Since the stadiums' grass didn't change and there was no referee effect, the most plausible explanation of this decrease is the absence of supporters that can influence the players and the referees. That hypothesis seems the most relevant since after the pandemic, the home advantage returned with the fans. The magnitude of the effect was generally similar although slightly smaller. Possible explanations for this slight decrease include teams becoming accustomed to playing in empty stadiums or changes in fan behavior before and after the pandemic.
...
Another deepening would be to explore other definitions of the home advantage. Indeed, we did not explore the yellow and red cards, which was studied by Garicano and Palacios-Huertas [1]. One could also fit a Poisson model counting the number of each according to the team playing home or away. Additionally, while we analyzed the number of goals in the linear regression model, it could also be interesting to investigate a wider indicator such as the number of targeted goals instead.
