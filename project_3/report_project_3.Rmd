...
```{r residuals 2019, echo=FALSE, include = FALSE}
#restrict the number of variables
...
```{r residuals all, echo=FALSE, include = FALSE}
par(mfrow=c(2,2))
...
|--------------|----------|-----------|---------|------------|
| Before covid | 0.28     | 0.09      | 2.86    | 0.004      |
| During covid | -0.07    | 0.13      | -0.51   | 0.61       |
| After covid  | 0.28     | 0.13      | 2.29    | 0.047      |
...
names_del <- unique(c(A, B, C, D)) #we will not take in account the ones that were relegated and promoted
