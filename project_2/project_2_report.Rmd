...
suppressWarnings(pairs(df$purchase ~ df$page_values + df$exit_rates + df$bounce_rates + df$time_prod_rel_page + df$n_prod_rel_page + df$n_admin_page + df$visitor_type, 
      labels = c("Purchase", "Page values", "Exit rates", "Bounce rates", "Time.prod.rel", "Num.prod.rel", "Num.admin", "Visitor type"),
...
#graphs not included in the report but that correspond to what is described below 
...
#productduration
df%>%group_by(purchase, time_prod_rel_page)%>% ggplot(aes(x=purchase, y=time_prod_rel_page)) + geom_boxplot()

#n_prod_rel_page
df%>%group_by(purchase, n_prod_rel_page)%>% ggplot(aes(x=purchase, y=n_prod_rel_page)) + geom_boxplot()

#time_info_page
df%>%group_by(purchase, time_info_page)%>% ggplot(aes(x=purchase, y=time_info_page)) + geom_boxplot()

#n_info_page
df%>%group_by(purchase, n_info_page)%>% ggplot(aes(x=purchase, y=n_info_page)) + geom_boxplot()

#time_admin_page
df%>%group_by(purchase, time_admin_page)%>% ggplot(aes(x=purchase, y=time_admin_page)) + geom_boxplot()

#n_admin_page
df%>%group_by(purchase, n_admin_page)%>% ggplot(aes(x=purchase, y=n_admin_page)) + geom_boxplot()
...
In summary, the clustered variables are as follows :
...
+ The log and square root of the exit rate
+ The log and square root of the bounce rates
+ The log and square root of the number of administrative pages consulted and the log and square root of the time spent on those pages
+ The log of the number of product related pages consulted and the log and square root of the time spent on each 
+ The log of the informative duration 
+ The log of the page values 

Finally, I added variables accounting for interactions between the number of pages visited and the time spent on each, for each category of pages (administrative, product related, informative).
...
df <- df%>%select(purchase,visitor_type,grouped_traffic_type,region,grouped_browser, grouped_operating_systems,page_values, exit_rates, bounce_rates, special_day, weekend, month, time_prod_rel_page, n_prod_rel_page, time_info_page, n_info_page,time_admin_page, n_admin_page)
...
df <- df %>% mutate(
  ...
  log_n_admin_page = log(n_admin_page +1),
  log_admin_duration = log(time_admin_page +1),
  log_prod_related_duration = log(time_prod_rel_page +1),
  log_prod_related = log(n_prod_rel_page +1),
  log_inf_duration = log(time_info_page+1),
  log_inf = log(time_info_page +1),
...
df <- df %>% mutate(
  ...
  sqrt_n_admin_page = sqrt(n_admin_page),
  sqrt_time_admin = sqrt(time_admin_page),
  sqrt_time_duration = sqrt(time_prod_rel_page)
)

#interaction terms
df <- df %>% mutate(
  inter_product = n_prod_rel_page * time_prod_rel_page,
  inter_n_info_page = n_info_page * time_info_page,
  inter_admin = n_admin_page * time_admin_page
)
```
...
m0_0 <- glm(purchase ~visitor_type + grouped_traffic_type + region + grouped_browser + grouped_operating_systems + page_values + exit_rates + bounce_rates + special_day + weekend + month + time_prod_rel_page + n_prod_rel_page + time_info_page + n_info_page + time_admin_page + n_admin_page, family = 'binomial', data = df) #without the new included variables 
...
model <- glm(purchase ~ visitor_type + grouped_traffic_type + 
    page_values + exit_rates + month + time_prod_rel_page + 
    n_admin_page + sqrt_exit + sqrt_bounce + sqrt_n_admin_page + 
...
improved_model <- glm(purchase ~ visitor_type + grouped_traffic_type + 
    page_values + exit_rates + bounce_rates + month + time_prod_rel_page + 
    n_admin_page + log_admin_duration +log_prod_related_duration + log_page_values + sqrt_exit + sqrt_bounce + sqrt_n_admin_page + 
    sqrt_time_duration, family = "binomial", data =df)
summary(improved_model)
Anova(improved_model, type = 'II', test ='LR') #sqrt_n_admin_page is not significant
...
improved_model_2 <- glm(purchase ~ visitor_type + grouped_traffic_type + 
    page_values + exit_rates + bounce_rates + month + time_prod_rel_page + 
    n_admin_page +log_prod_related_duration + log_page_values + sqrt_exit + sqrt_bounce  + 
    sqrt_time_duration, family = "binomial", data =df)
...
I then use the stepAIC function of the MASS package to perform AIC-based model selection. This creates a basis model that I improve by examining the significance of the variables with the Anova command. This allows me to build an improved model by adding variables one by one and simultaneously testing their significance.
...
| AUC                | 0.884  | 0.911  | 0.889  | 0.913  |
| Residuals deviance | 7059   | 5938   | 7030   | 5974   |


# Discussion
In conclusion, the logistic regression model developed in this project is satisfactory with all variables being significant at the 5% level. The model also outperformed other models based on AIC and AUC, and performed among the best ones regarding residuals deviance, which instills confidence in its accuracy. However, the presence of influential outliers in the dataset warrants further investigation, and their removal could potentially lead to an even more improved model. 
