...
#browser
df%>%group_by(grouped_browser, purchase)%>%summarise(number=n()) %>% mutate(freq = 100*number / sum(number)) %>% 
  ggplot(aes(x=browser, y=freq, fill = purchase )) + geom_bar(stat="identity", position ='dodge')

#traffic_type
df%>%group_by(grouped_traffic_type, purchase)%>%summarise(number=n()) %>% mutate(freq = 100*number / sum(number)) %>% 
