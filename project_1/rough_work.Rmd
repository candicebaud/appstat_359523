...
```{r parametric bootstrap 3, echo=FALSE}
...
T_stat <- function(xn, mu1, mu2, sigma1, sigma2,p){
  ...
  sample <- rep(0,n)
  for (i in 1:n){
    sample[i] <- bilognorm(grid[i], mu1, mu2, sigma1, sigma2, p)
  }
  F_star <- ecdf(sample)
  #difabs <- abs(F_N(grid) - plnormMix(grid, meanlog1 = abs(mu1), sdlog1 = abs(sigma1), meanlog2 = abs(mu2), sdlog2 = abs(sigma2), p.mix = p) )
  difabs <- abs(F_N(grid) - F_star(grid) )
  return(max(difabs))
}


Bootstrap <- function(xn, mu1, mu2, sigma1, sigma2,p, B){
  n = length(xn)
  #first we utilize our sample ie df$retained....
  param = estimate_2(xn, p, mu1, mu2, sigma1, sigma2)
  mu1_estim <- param[1]
  mu2_estim <- param[2]
  ...
  #Bootstrap
  T_boot <- rep(NA, B)
  x_resamples <- matrix(NA, nrow = B, ncol = n)
  for (i in 1:B) {
  ...
  return(1/(B+1)*(1 + sum(Tb[(Tb>T_0)])))
}

Bootstrap(X, mu1, mu2, sigma1, sigma2,p, 1000)
...
Param_bootstrap <- function(xn, Bboot, mu1_init, mu2_init, sigma1_init, sigma2_init, p_init){
  ...
  return(1/(Bboot+1)*(1 + sum(Tb[(Tb>T0)], na.rm = T )))
}
