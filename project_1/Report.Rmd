...
| Setup 4 | $\mu_1 - \epsilon$ | $\mu_2+\epsilon$ | $\sigma_1+\epsilon$ | $\sigma_2+\epsilon$ | $p-\epsilon$ |
...
Throughout this project, I have focused on the mixture of two lognormal distributions as it fits the data well, and my parametric bootstrap algorithm did not reject it. The p-value of 1 is doubtable but explained by the fact that I only did  10 bootstrap resamples due to the computational cost of my algorithms. Doing it for more than 5000 times gives a p-value of 0.997 which gives a very high level of confidence about the fact that the distribution comes from a mixture of two lognormal distributions. Estimating the parameters of this distribution is challenging, as there is no closed-form formula for the maximum likelihood estimate. I have used a combination of the EM algorithm and local optimization to estimate the parameters, which resulted in a slightly improved fit of the model. The initial parameters for the EM algorithm were chosen based on descriptive statistics and provided a satisfactory starting point for the optimization. 
