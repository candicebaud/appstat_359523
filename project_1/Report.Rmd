...
The log likelihood doesn't have a good form so we draw $Zn = \mathbb{I}_{\left[X_n \text{ drawn from }\varphi_{\mu_2,\sigma_2^2}\right]} \sim \mathrm{Bern}(p)$ iid and independent of the $X_i$. We can rewrite the likelihood as \[
...
Before doing my EM-algorithm, I do some jittering. I simulate points uniformly in the different diameters intervals in a proportion according to the percentage of particles in each bin. Although the number of detected particles is 705044, here I only simulate 705 to have a faster algorithm and because the results aren't very different using all the points.

```{r EM algo jittering, echo=FALSE}
#simulate points 
n_points <- trunc(df$particles.detected[1]/1000) #total of points I will take
X <- c()
...
With the parameters I obtained, I generate *Figure2* which shows the density of the fitted model and our data. The red curve represents the data we aim at modeling and the black one represents our model estimation. We will keep this convention throughout the report.
...
Although the density I obtained approximately fits the data, it is not fully satisfying because the peak at 0.7 is weaker than in the true distribution, thus is not accurately modeled. Moreover, the peak at 0.25 is over-estimated. To improve the fit, we will consider using local optimization techniques.
...
After re-evaluating the parameters, I refit the model to see if there were any improvements. *Figure4* shows the results with an histogram. There is a true improvement in our estimation thanks to the use of kernel density estimation. Specifically, one can see that the peak at 0.6 is better estimated, suggesting that our modeling approach is capturing more of the underlying distribution. However, I note that the peak at 0 is still over-estimated, which indicates that further optimization may be necessary to obtain an accurate model.
...
*Figure6* allows us to see the new fitted distribution which doesn't appear better than the previous one. The generated histogram doesn't represent the true distribution accurately at all since the peaks are not accurately represented. I will discuss those results deeper in the following section.
...
The obtained p-value is of 0.99 so we don't reject the null hypothesis. In other words, we conclude that the hypothesis that our data comes from a mixture of two lognormal distributions is not rejected.
