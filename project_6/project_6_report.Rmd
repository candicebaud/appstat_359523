...
# Introduction
The covid-19 pandemic which started in December 2019 in Wuhan had a significant impact on the world, with over 160 million confirmed cases and 3.3 million deaths worldwide as of May 2023. The disease has been studied extensively in various fields, including epidemiology, medicine, and public health, and many models have been proposed. Some were based on theoretical models (SIR, SEIR, SEIRD...) and fitted to the data by estimating the models parameters. Another way of modelling the problem is to base the analysis on the data itself and not a general model, which has been proposed by Marc Lavielle [1], by detecting changing points in the derivative of the number of hospitalized patients, and then fitting polynomial regressions. In this project, we want to explore the pandemic's amplitude variation with pca. To this end, we must consider the fact that countries were not impacted at the same time by the virus and thus synchronize the countries in a different time scale.  

We will begin by introducing the data collection and its characteristics and then ways to smooth it and synchronize curves. Next, we will perform functional pca on the smoothed data. Finally, we will interpret the results.
...
df <- df %>% mutate(
  log_total_cases = log10(total_cases),
  log_total_cases_per_million = log10(total_cases_per_million)
)
...
# Data modification 
## Method 1 
One approach I consider is to first smooth the data and then align the curves by setting a common starting point based on a specific number of cases. Each country will have a fixed number of observations, but they will not end at the same time since they don't start at the same time.
...
### Shifting the curves 
Previously, we applied smoothing techniques to the curves; however, they are currently not synchronized, requiring us to adjust their alignment. I have established a threshold based on the number of accumulated cases. Once a country surpasses this threshold, I redefine the time scale, ensuring that all curves start at that specific time. Consequently, the curves are shifted to commence together when they have reached a similar stage in the pandemic's progression. The outcomes of this alignment can be observed in *Figure 3*.
...
```{r, fig.align="center", fig.cap="Figure 3: Shifted and smoothed curves - method 1"}
...
The alignment of the curves successfully ensures that they start at the same time and exhibit a similar trend simultaneously. However, it is important to note that variations in their evolution still persist, as evidenced by the lack of complete synchronization in the peaks. Moreover, I had to truncate some values which in the end which doesn't seem a good idea. In the next subsection, I implement another method. 


## Method 2
The second method I consider is to first synchronize my curves and then smooth it with B splines.

### Shifting the curves 
To ensure consistency among the curves, I aim to align them while preserving all data points up until the specified limit of November 1, 2020. To achieve this, I normalize the time scale, representing the entire study period as the interval [0,1]. Here, 0 corresponds to the individual starting points (which vary across countries), and 1 represents the fixed end point of November 1, 2020, common to all countries.
...
```{r, fig.align="center", fig.cap="Figure 4: Shifted curves - method 2"}
data_meth_2 %>% group_by(location) %>% ggplot(aes(x=time, y=roll_log_cases_per_million, color=location)) + geom_line() +ylab('Logarithm of the number of cases per million')

``` 

### Smoothing
I don't have the same amount of observations for each country and I have not yet smoothed my data so I use the B-splines to do so. I have almost 250 values for each country, so I decide to do evaluate my B-splines on 150 final values. After smoothing, I obtain *Figure 5*.


```{r autre méthode, echo = FALSE, include = FALSE}
library(fda)
col = c('time', 'log_total_cases_per_million', 'location')
data_pca <- data_meth_2[,col]

data_smoothed <- list()
z <- seq(0, 1, length = 150)
for (i in 1:length(countries)){
  df_country <- data_pca[data_pca$location == countries[i],]
  nodes_per_week <- c(seq(0,1, by = 7/length(df_country$time)), 1)
  B_base <- bsplineS(df_country$time, norder = 6, nodes_per_week)
  eta_hat <- ginv(t(B_base)%*% B_base)%*%t(B_base)%*% df_country$log_total_cases_per_million

  Bplot <-bsplineS(z, norder = 6, nodes_per_week)
  data_smoothed[[i]] <- as.vector(Bplot %*% eta_hat)
}

data_smoothed <- data.frame(t(do.call(rbind, data_smoothed)))
colnames(data_smoothed) <- countries

``` 



```{r, fig.align="center", fig.cap="Figure 5: Shifted and smoothed curves - method 2"}
to_plot <- data_smoothed %>% mutate( time = seq(1, length(data_smoothed[,1]), by = 1))
to_plot <- to_plot%>% pivot_longer(!time, names_to = "location", values_to = "total_cases_log_smoothed")


to_plot %>% group_by(location)%>% ggplot(aes(x = time, y = total_cases_log_smoothed, color = location)) + geom_line() + ylab('Smoothed logarithm of the total cases')

rm(to_plot)
```

Another intriguing approach for shifting the curves would have been by following Marc Lavielle's procedure [1]. In his article, Lavielle utilizes the logarithmic derivative (u'/u) of the number of daily cases to identify critical breaking points in the curve. These breaking points play a significant role in determining the pandemic's progression towards a peak or a decline in the number of cases. By selecting and aligning the breaking points, it becomes possible to shift the curves together, ensuring that the breaking points coincide. However, this method would probably fail in this case because of the fact that governments influenced the pandemic's evolution in different ways. Indeed, the lockdowns were not of the same intensity in all the countries, and even some Nordic countries did not choose to confine their population.

# PCA
## With the 1st method
Now that the countries are synchronized in time, I do PCA on the smoothed logarithm of the cumulated number of cases. *Figure 4* displays the 4 first PCA obtained using the function svd().
...
#gg_miss_fct(x = data_pca, fct = time) check missing data
...
```{r, fig.align="center", fig.cap="Figure 5: PCA curves with method 1"}
X = t(matrix_pca)
...
## With the 2nd method 
I represent in *Figure 6* the PCA obtained with the second method. 
```{r, fig.align="center", fig.cap="Figure 6: PCA curves with method 2"}
col <- countries
...
# Interpretation 
The results of the principal component analysis (PCA) with method 1 reveal that the first principal component accounts for 81% of the total variance. This component exhibits an initial steep upward trend, capturing the first peak observed in the curves. Towards the end, it gradually decreases until it reaches zero. The second principal component explains 14% of the variance and shows an increasing trend following the decline of the first principal component. Moving on, the third and fourth principal components display a peak at the beginning, followed by a decrease. Together, they account for 4% of the explained variance. In total, the first four principal components explain 99% of the total variance observed in the data.


The second principal component analysis obtained with method 2 reveals that the first principal component also accounts for 79% of the total variance and is really close to what I had obtained with method 1. The second principal component analysis accounts now for 14% of the variance, whereas the third and fourth account for 6%. In total the first four PC account for 99% of the variance.
...
To address this time shift, we adjusted the curves by synchronizing them, enabling us to conduct a more accurate principal component analysis (PCA). By considering the curves' alignment, with method 1, we were able to obtain the four leading principal components, which accounted for 99% of the total variance. Notably, the first principal component explained 81% of the variance and effectively captured the initial peak experienced by the countries. However, it is important to acknowledge that factors such as countries' demographics and political measures can significantly influence the pandemic's evolution. This complexity limits the effectiveness of our simple curve-shifting method, which assumes linear time progression.

With the second method which also addresses the issue of the time shift, the PCA gives similar results with close explanatory power of the principal components. Indeed, the time window that was chosen corresponds to the 1st peak of the pandemic, thus, countries had close dynamical evolutions. Then, countries implemented different measures that changed the local behavior of the pandemic inside the country and which created different and lagged evolutions. 

The good results obtained by our PCA without doing proper registration with monotonic function are mostly explained by the fact that we studied a process relatively common to all countries. 

A deepening of this work would be to analyze the PCA according to countries specific factors such as the GDP per capita, life expectancy, or a stringency index representing the covid-19 measures taken by the governments. 
