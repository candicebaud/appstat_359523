...
estimate <- function(p_init, mu1_init, mu2_init, sigma1_init, sigma2_init){
...
param <- estimate(p_init = p_init, mu1_init = mu1_init, mu2_init =mu2_init, sigma1_init = sigma1_init, sigma2_init = sigma2_init )
...
for (i in 2:n_iter_max) {
  ...
  log_ratio <- likelihood_mixed(X, prop[1], prop[2], prop[3], prop[4], prop[5]) - likelihood_mixed(X, chain[i-1,][1], chain[i-1,][2], chain[i-1,][3], chain[i-1,][4], chain[i-1,][5])
  accept_prob <- exp(log_ratio)
...
sample_bilog <- function(n, mu1_init, mu2_init, sigma1_init, sigma2_init, p_init){
  for (i in 1:52){
    sample[i] <- p_init * rlnorm(n, meanlog = mu1_init, sdlog = sigma1_init) + (1-p_init) * rlnorm(n, meanlog = mu2_init, sdlog = sigma2_init)}
  return(sample)
}
...
Param_bootstrap <- function(xn, Bboot, mu1_init, mu2_init, sigma1_init, sigma2_init, p_init){
  ...
  for (b in 1:Bboot){
    ...
    xb_star <- sample_bilog(n, mu1_init, mu2_init, sigma1_init, sigma2_init, p_init)
    #Estimate parameters of the sample
    estim <- estimate(xb_star) #do the function estimate
    mu1_init <- estim[1]
    mu2_init <- estim[2]
    sigma1_init <- estim[3]
    sigma2_init <- estim[4]
...
Param_bootstrap(X, Bboot = 50, mu1_init = mu1_init, mu2_init = mu2_init, sigma1_init = sigma1_init, sigma2_init = sigma2_init, p_init = p_init)
